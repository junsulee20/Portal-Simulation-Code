...
class RealizedDRTSimulation:
    ...
    def process_request(self, request: PassengerRequest, request_time: float) -> Optional[AssignmentEvent]:
        ...
        """
        # request_time을 요청 객체에 주입
        request.request_time = request_time
        
        # 배정 처리 (current_time 매개변수로 request_time 전달)
        assigned_vehicle, new_path, cost = self.engine.assign_request(self.vehicles, request, request_time)
        ...
        wait_assign = max(0.0, assignment_time - request_time)
        wait_pickup = max(0.0, pickup_time_from_start)
