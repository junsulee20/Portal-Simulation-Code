...
VEHICLE_LOG_CSV_PATH = "vehicle_detail_log_120.csv"  # 요청별 상세 의사결정 로그
...
class AssignmentEvent:
    ...
    original_path_time: float  # 배정 전 차량 경로 이동 시간 (초)
    # ── 의사결정 신호 3종 ──────────────────
    pickup_rank: int           # 신규 경로에서 이 승객 픽업이 몇 번째 stop인지 (0-indexed)
    dist_veh_to_pickup: float  # 차량 현재 위치 → 픽업 노드 이동 시간 (초) — '가까운 차량' 지표
    detour_ratio: float        # cost_increase / original_path_time — '경로 굽힘' 지표 (0=우회 없음)
...
def run_one_simulation(
    ...
    for demand in fixed_demands:
        ...
        # ── 의사결정 신호 3종 계산 ─────────────────────────────────────────────────
        # 1) pickup_rank: 신규 경로에서 이 승객 픽업이 몇 번째 stop인지
        #    숫자가 작을수록 픽업을 '앞당긴' 것
        pickup_rank = next(
            (i for i, s in enumerate(new_path)
             if s.passenger_id == request.passenger_id and s.stop_type == "pickup"),
            -1,
        )
        # 2) dist_veh_to_pickup: 차량 현재 위치 → 픽업 노드 이동 시간
        #    작을수록 '가까운 차량'이 배정된 것
        dist_veh_to_pickup = engine.travel_time_cache.travel_seconds(
            start_node, request.pickup_node
        )
        # 3) detour_ratio: cost_increase / original_path_time
        #    클수록 기존 경로를 '더 굽힌' 것
        detour_ratio = (
            cost_increase_val / original_path_time
            if original_path_time > 0 and not math.isinf(original_path_time)
            else 0.0
        )
        # ────────────────────────────────────────────────────────────────────────
        ...
        event = AssignmentEvent(
            ...
            original_path_time=original_path_time if not math.isinf(original_path_time) else 0.0,
            pickup_rank=pickup_rank,
            dist_veh_to_pickup=dist_veh_to_pickup if not math.isinf(dist_veh_to_pickup) else 0.0,
...
def save_vehicle_log(results: List[Dict], path: str) -> None:
    """
    요청별 의사결정 상세 로그를 CSV로 저장.

    컬럼 설명:
        run_label       : 가중치 조합 레이블 (BASELINE 또는 (w1,w2,w3))
        w1/w2/w3        : 가중치
        req_id          : 승객 ID
        vehicle_id      : 배정된 차량 ID
        wait_assign     : 요청→배정 대기 시간 [s]
        wait_pickup     : 배정→픽업 ETA [s]
        total_wait      : 총 대기 시간 [s]
        dist_veh_pickup : 차량 현재 위치→픽업 이동 시간 [s]  → '가까운 차량' 지표
        pickup_rank     : 신규 경로에서 픽업이 몇 번째 stop? → '픽업 앞당기기' 지표
        stops_before    : 배정 전 차량 경로 stop 수
        stops_after     : 배정 후 차량 경로 stop 수
        original_pt     : 배정 전 경로 이동 시간 [s]
        cost_increase   : 경로 증가 시간 [s]
        detour_ratio    : cost_increase / original_pt → '경로 굽힘' 지표
        new_path_time   : 배정 후 전체 경로 시간 [s]
        travel_time     : 픽업→드롭오프 시간 [s]
    """
    fieldnames = [
        "run_label", "w1", "w2", "w3",
        "req_id", "vehicle_id",
        "wait_assign", "wait_pickup", "total_wait",
        "dist_veh_pickup", "pickup_rank",
        "stops_before", "stops_after",
        "original_pt", "cost_increase", "detour_ratio", "new_path_time",
        "travel_time",
    ]
        ...
        for r in results:
            for e in r["events"]:
                writer.writerow({
                    ...
                    "w2":             r["w2"],
                    "w3":             r["w3"],
                    "req_id":         e.request.passenger_id,
                    "vehicle_id":     e.vehicle_id,
                    "wait_assign":    round(e.wait_assign,        2),
                    "wait_pickup":    round(e.wait_pickup,        2),
                    "total_wait":     round(e.total_wait,         2),
                    "dist_veh_pickup": round(e.dist_veh_to_pickup, 2),
                    "pickup_rank":    e.pickup_rank,
                    "stops_before":   len(e.previous_path),
                    "stops_after":    len(e.new_path),
                    "original_pt":    round(e.original_path_time, 2),
                    "cost_increase":  round(e.cost_increase,      2),
                    "detour_ratio":   round(e.detour_ratio,       4),
                    "new_path_time":  round(e.new_path_time,      2),
                    "travel_time":    round(e.travel_time, 2)
                                      if not math.isinf(e.travel_time) else "",
                })
    print(f"\n📋  요청별 상세 로그 CSV 저장 완료: {path}\n")
...
def main() -> None:
    ...
    # ── 요청별 상세 의사결정 로그 (baseline + 최적해만) ─────────────────────────────
    log_runs = [results[0]]  # baseline (항상 포함)
    if best is not None and best is not results[0]:
        log_runs.append(best)   # 최적 가중치 결과 추가
    save_vehicle_log(log_runs, VEHICLE_LOG_CSV_PATH)

    print("※ 상세 통계는 위에서 저장된 CSV 파일을 참고하세요.")
    print("  결과 요약 CSV   : " + RESULT_CSV_PATH)
    print("  요청별 로그 CSV : " + VEHICLE_LOG_CSV_PATH)
    print()
    print("[요청별 로그 컬럼 해설]")
    print("  pickup_rank     → 낮을수록 '픽업 앞당김' 선택")
    print("  dist_veh_pickup → 낮을수록 '가까운 차량' 배정")
    print("  detour_ratio    → 높을수록 '경로를 더 굽힘'  (cost_increase / original_pt)")
