"""
고정된 디멘드와 차량 초기 위치를 사용하는 DRT 배정 시뮬레이션 및 시각화 모듈 (성능 최적화 버전).
...
# 디멘드 개수 설정 (여기서 변경하세요!)
# ================================================================================
NUM_DEMANDS = 10  # 디멘드 개수 (최대 100개까지 설정 가능)
# ================================================================================
# --------------------------------------------------------------------------------------
...
class FixedDemandSimulation:
    """고정된 디멘드를 사용하는 DRT 배정 시뮬레이션 (성능 최적화 버전)."""
    ...
    def process_request(self, request: PassengerRequest, request_time: float) -> Optional[AssignmentEvent]:
        ...
        # 성능 최적화: 경로 노드 구축 제거 (시각화 단계에서만 수행)
        # prev_route_nodes = build_route_nodes(self.graph, start_node, previous_path)  # 제거됨
        # new_route_nodes = build_route_nodes(self.graph, start_node, new_path)  # 제거됨
...
def main() -> None:
    """메인 실행 함수."""
    print("=" * 70)
    print("DRT 네트워크 배정 시뮬레이션 (고정된 디멘드) - 성능 최적화 버전")
    print("=" * 70)
    print()
